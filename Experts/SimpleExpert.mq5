...
class CEdit final : public DrawableObject {
 ...
 ~CEdit() {
  ...
  void Draw() {
    ...
    if (BoolInit) {setText(Function.Init()); BoolInit = false;}
  }
  ...
  bool EventButton(string const& sparam) {
    if (Trade.name == sparam) {
      if (DrawDeals.State()) {
        ...
        ListEdits.Hide();
        for (uint i{0}; i < DrawDeals.SizeCounter(); ++i) {TradeLine(i);}
        Reset.Draw();
      }
      ResetBool = true;
      return true;
      ...
      if (!draw_deals) {Trade.Hide(); DrawDeals.DeleteLines();}
      if (!draw_positions) DrawPositions.DeleteLines();
  ...
  void CalcLevels() const {
    ...
    if (state_deals || state_positions) {
      ...
      while (Margin * MarginCall < equity && total_volume < Volumes.VolumeLimit && fmax(DrawDeals.SizeCounter(), DrawPositions.SizeCounter()) < AccountInfoInteger(ACCOUNT_LIMIT_ORDERS)) {
        if (state_deals    ) DrawDeals    .Push(price                      ,       volume);
        if (state_positions) DrawPositions.Push(total_notional/total_volume, total_volume);
    ...
    if (state_deals    ) DrawDeals    .DrawLines();
    if (state_positions) DrawPositions.DrawLines();
  }
...
class CSpread final {
 public:
  ...
  CSpread()
    ...
    if(SymbolInfoInteger(Symbol(), SYMBOL_SPREAD_FLOAT)) String=StringFloatSpread;
    else                                                 String=StringIntegerSpread;
  }
};
